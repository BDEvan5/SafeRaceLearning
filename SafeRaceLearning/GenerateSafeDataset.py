...
from SafeRaceLearning.SafetyDNN.DataStateHistory import DataStateHistory
...
from SafeRaceLearning.Planners.follow_the_gap import FollowTheGap
from SafeRaceLearning.Planners.AgentPlanners import AgentTester
from SafeRaceLearning.Planners.RandomPlanner import RandomPlanner, RandomFeasiblePlanner
...
class TestSimulation():
    ...
    def run_testing_evaluation(self):
        for run in self.run_data:
            ...
            elif run.architecture == "RandoData": 
                planner = RandomFeasiblePlanner(run, self.conf)
            ...
            self.vehicle_state_history = DataStateHistory(run)
        ...
        for i in range(self.n_test_laps):
            ...
            while not observation['colision_done'] and not observation['lap_done']:
                ...
                if self.vehicle_state_history: 
                    self.vehicle_state_history.add_memory_entry(observation, action)
            ...
            if self.vehicle_state_history: self.vehicle_state_history.save_history()
